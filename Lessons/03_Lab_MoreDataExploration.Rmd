...
```{r DataExercise-1a}
highf <- c(54,71,72,63,51,52,61,54,67,73)
highl <- c(42,49,60,46,32,52,59,43,55,49)
```
...
```{r DataExercise-1b}
temp_conversion <-function(F)
f2c <- function(F)
{c<-round((f-32*5/9),2) return (c)}
highc <-f2c(highf)
lowc<-f2c(lowf)
```
...
ifelse(year(date_obj_today) %%100 >23, year(date_obj_past)-100, year(date_obj_past)) #this one is wrong, fix it 
